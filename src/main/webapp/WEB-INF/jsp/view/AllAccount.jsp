...
<div class="w3-content w3-padding" style="max-width:1564px">
  ...
  <div class="w3-container w3-padding-32" id="about">
    <h3 class="w3-border-bottom w3-border-light-grey w3-padding-16">Account List</h3>
    <p>  
        ...
        <c:if test="${fn:length(users) > 0}">
            <table border = "1"  style="font-size: 20px;" >
                <tr><th>Username</th><th>User Role</th><th>Action</th></tr>
                        <c:forEach var="entry" items="${users}">
                    <tr><td> ${entry.username}</td><td>
                            <c:forEach var="role" items="${entry.roles}" varStatus="status">
                            ...
                            [<a href="<c:url value="/admin/account/edit?username=${entry.username}" />">Edit</a>] 
                            [<a href="<c:url value="/admin/account/delete?username=${entry.username}" />">Delete</a>]</td></tr>
                        </c:forEach>
            </table>


        </c:if>
        <p><a href="<c:url value="/admin/account/add" />" class="w3-button w3-black w3-section">Add Account</a> <a href="<c:url value="/admin" />" class="w3-button w3-black w3-section">Back to Admin Page</a></p>
